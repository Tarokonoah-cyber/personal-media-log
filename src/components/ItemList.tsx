...
function privateColumnDefs({ onQuickUpdate }: { onQuickUpdate?: (item: MediaItem, patch: Partial<ItemInput>) => Promise<void> }): ColumnDef[] {
  return [
    ...
    { id: "used", label: "狀態", colClassName: "private-used-col", cellClassName: "private-used-cell", render: (item) => <PrivateUsedToggle item={item} onQuickUpdate={onQuickUpdate} /> },
...
function PrivateUsedToggle({ item, onQuickUpdate }: { item: MediaItem; onQuickUpdate?: (item: MediaItem, patch: Partial<ItemInput>) => Promise<void> }) {
  ...
  return (
    <label className="private-used-toggle" onClick={(event) => event.stopPropagation()} title={used ? "完成" : "待處理"}>
      <input
        type="checkbox"
        checked={used}
