...
export type PrivateClipboardUpdate =
  ...
  | { kind: "quick"; field: "rating" | "collection_level"; value: number | CollectionLevel | null };
...
export function privateClipboardValue(item: MediaItem, column: PrivateColumnId) {
  ...
  if (column === "rating") return item.rating === null || item.rating === undefined ? "" : String(item.rating);
  if (column === "favorite") return collectionLevelLabels[normalizeCollectionLevel(item.collection_level ?? item.favorite_level)];
...
export function privateClipboardUpdate(item: MediaItem, column: PrivateColumnId, rawValue: string): PrivateClipboardUpdate {
  ...
  if (column === "rating") {
    ...
    if (!Number.isFinite(rating) || rating < 1 || rating > 10) throw new Error("評分必須是 1–10");
    return { kind: "quick", field: "rating", value: rating };
  }
  if (column === "favorite") {
    return { kind: "quick", field: "collection_level", value: parseCollectionLevel(value) };
  }
  if (column === "watchedAt" && value && !isValidIsoDate(value)) throw new Error("紀錄日格式必須是 YYYY-MM-DD");
...
function parseCollectionLevel(value: string): CollectionLevel {
  const normalizedKey = value.toLocaleLowerCase();
  const key = collectionLevels.find((level) => level === normalizedKey);
  if (key) return key;
  const label = collectionLevels.find((level) => collectionLevelLabels[level] === value);
  if (label) return label;
  throw new Error("收藏必須是未分類、神作、一般或淘汰");
}
